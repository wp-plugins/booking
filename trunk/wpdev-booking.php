...
/*  Copyright 2009, 2010  www.wpdevelop.com  (email: info@wpdevelop.com),
...
// <editor-fold defaultstate="collapsed" desc=" TODO: & Changelog lists ">
/*
-----------------------------------------------
 ...
 * On hover over date, booked time shows as 24-mode, have tobe this in  12-hour with am/pm
 ...
 * ·         Ressource 4
 * -------------------------------------------------------
 * The email exchanges with the client should always mention a reservation number
 * the total value of booking to be visible before they send booking
 * ??? the payment request to be invisible and be sent by email once the booking is approved.
 * check posibility to include this payment system : https://www.nmi.com/ (its for Panama)
 * The system is applying the partly booked icon to the wrong day. Its appear after book button is pressed.
 ...
 * Add posibility to select only weeks (or may be other number of days). Its mean that can be selected only 7+7+7+... days.
 ...
 * country field - default country selection
 * More good look of help info at the form fields customization page
 * more options at the popup during inserting shortcodes to the post or page
 * you are booking into a Hotel and you want one Double Room, Two Single Rooms and a Conference Room for 3 consecutive dates... You want to do this with one form and put in your details only once on one form.
 ...
 * Posibility to set thank you page instead of thank you message. Its usefull, for exmaple, when need to use google analitycs for the bookings checking.
 * This is ok but i can see that there is a problem that you can see in the "language2.jpg" attachment. Where can i change the word "Cost:" to "Costo:"  ? ....and add a <br> tag before paypal button?. Check trsnslation of monthes inside of calendar.
 * Show bk resource in more good way
 *
 ...
 *   feature pages,
 *   demo pages,
 *   wp-login at onl.com site
 * also update
 *   .net and
 *   .net/purchase page
 *
 * make update of free version
 *
 * test free version data 
...
= 2.6 =
* Professional / Premium / Premium Plus / Hotel version features:
...
= 2.5 =
* Professional / Premium / Premium Plus / Hotel version features:
 * New Hotel Edition. This version is completly redeveloped. Now Booking Resource (which have capacity per day for reservation) can conatin independent CHILD resources. After reservation is done, booking are store inside of one specific child booking resource. Each child booking resource have priority, its mean that items (booking resources, like rooms or seats...) with higher priority will reserved firstly, then booking resources with lower priority. ( Hotel Edition )
 ...
 * Calculation of final cost based on % from the number of selected days for booking (option "Together" at the payemnt configuration settings page, where cost depends from number of selected days ) (Premium Plus, Hotel Edition)
 ...
 * Posibility to show reserved days at one calendar from several booking resource - agregation calendar, this calendar have own booking resourse, but make unavailable some days based on resources from other calendars. So its mean that one calendar can aggregate dates from several booking resource. Usage: Now available new parameter for [booking] shortcode - agregate='2;4;5', where 2;4;5 - ID of booking resources. Example: [ booking nummonths=2 type=3 form_type='standard' agregate='1;2' ]  (Professional, Premium, Premium Plus, Hotel Edition)
 ...
 * Translation into Italian language by <a href="http://www.icreation.it" target="_blank">Edoardo Langasco</a>
 ...
 * Fixed issue for cost calculation, which is depends from number of selected days for booking. Issue was with option "Together". ( Hotel Edition )
 ...
 * Cost implementation into email (Premium, Hotel Edition)
* Features and issue fixings in All versions:
 * CSS optimisation
...
* Features and issue fixings in All versions:
 * Admin booking email modification. From now toy can change admin email for bookings at the settings page at Booking Calendar Standard version (free version).
...
= 1.7.1 =
* Professional / Premium / Hotel version features:
 * Posibility to enter direct cost of a day/night/hour in rate season filter.  Direct curency rate type have top priority in seson filters. For example: we have several seson filters: weekends, summer and winter. And we are have booking resource wih day cost $100, and we are set next rates: Weekend = 50%, Winter 75%, Summer = $300. So then its mean if we are select WORK DAY at Winter its will cost $100 + 75% = $175, if we are select WeekEnd day at winter $100 + 75% + 50% = $262,5  but if we select workday or weekend at the summer its will return $300, because of top priority for curency rate type.
* Features and issue fixings in All versions:
 ...
 * Admin edit bookings functionality - from now at paid versions you can edit booking (for some corrections), which was made by visitor of your site. Email according modifications is also will send to the visitor of site, after saving of modification.
 * New email template, which is sending after booking modifications are done by admin.
...
= 1.5.3 =
* Professional / Premium / Hotel version features:
 * Showing daily cost at the highlighting tooltip ( Hotel Edition )
* Features and issue fixings in All versions:
 ...
 * Features and issue fixings in All versions:
  ...
  * Correct showing translation for the dates inside email and booking tables.
 ...
 * Professional version features:
  ...
  * Email customization form support new bookmrak - [name]. Its name of person, who made reservation.
...
if (!class_exists('wpdev_booking')) {
    class wpdev_booking {
        ...
        function wpdev_booking() {
            ...
            if ( ( strpos($_SERVER['REQUEST_URI'],'wpdev-booking.phpwpdev-booking')!==false) &&
                ...
                if (WP_ADMIN === true) wp_enqueue_script( 'jquery-ui-dialog' );
        ...
        <script type="text/javascript">
            ...
            if( $this->wpdev_bk_pro !== false ) {
                ...
                if( $this->wpdev_bk_pro->wpdev_bk_premium !== false ) {
                    $verion_width = '310';
                    if (  (class_exists('wpdev_bk_hotel')) || (class_exists('wpdev_bk_premium_plus')) ) {
                        $verion_width = '380';
                    }
                }
  ...
  style="<?php if  (isset($tl->parent)) if ($tl->parent == 0 ) { echo 'font-weight:bold;'; } else { echo 'font-size:11px;padding-left:20px;'; } ?>"
    ...
    width:350px;}
        ...
        function booking_table($approved = 0, $bk_type = 'not_set') {
...
//debuge($result)            ;
            ...
            foreach ($result as $res) {
                ...
                if ($old_id == $res->booking_id ) { // add only dates
                    ...
                    $result_structure[$ns]['type_id'][] = $res->type_id;
                } else { // new record
                    $ns++;
...
<div id="ajax_respond"></div>
            ...
            <?php
//TODO: HERE_EDITED
            if (($_GET['booking_type']=='-1') && ( $this->wpdev_bk_pro !== false )) {
...
<div id="table_approved">
    ...
    <div class="clear" style="height:20px;"></div>
        ...
        function settings_menu_top_line() {
            ...
            <?php } else {
                     if  (! isset($_GET['tab'])) $_GET['tab'] = 'form'; // For multiuser - common user set firt selected tab -> Form
            } ?>
            ...
            if ( ($version !== 'free') && ($version!== 'hotel') ) { $this->showUpgradeWindow($version);
                ...
                ?>
                    ...
                    <div class='meta-box-sortables ui-sortable' id="upgrade-box">
                        <div <?php $my_close_open_win_id = 'bk_upgrade-box-title'; ?>  id="<?php echo $my_close_open_win_id; ?>" class="postbox <?php if ( '1' == get_user_option( 'booking_win_' . $my_close_open_win_id ) ) echo 'closed'; ?>" > <div title="<?php _e('Click to toggle','wpdev-booking'); ?>" class="handlediv"  onclick="javascript:verify_window_opening(<?php echo get_bk_current_user_id(); ?>, '<?php echo $my_close_open_win_id; ?>');"><br></div>
                            <h3 class='hndle'><span>Upgrade to <?php if ( ($version == 'pro') ) { ?>Premium /<?php } ?> Hotel Edtion versions</span></h3>

                            <div class="inside">

                                <p>You can make <strong>upgrade</strong> to the <?php if ( ($version == 'pro') ) { ?><span style="font-weight:normal;">Booking Calendar Premium</span> or<?php } ?> <span style="font-weight:normal;">Booking Calendar Hotel Edition</span>: </p>
                <?php if ( ($version == 'pro') ) { ?>
                    ...
                    <?php } ?>
                                ...
                                <p style="padding:0px 10px;">
                                        ...
                                        &bull; <strong>Availability</strong> Set for each booking resource (un)avalaible days.<br/>
                                        &bull; <strong>Rates</strong> Set higher costs for peak season or discounts for specific days.<br/>
                                        &bull; <strong>Advanced costs</strong> Set additional costs, which will depends from selection in dropdown lists or checkboxes.<br/>
                                </p>
                <?php if( strpos( strtolower(WPDEV_BK_VERSION) , 'multisite') !== false  ) {
                    $multiv = '-multi';
                } else {
...
<script  type="text/javascript">
    ...
    var user_unavilable_days = [];
            ...
            //if (isset(get_bk_option( 'booking_skin'))) {
            ...
            if($is_admin) { ?> <link href="<?php echo WPDEV_BK_PLUGIN_URL; ?>/css/admin.css" rel="stylesheet" type="text/css" />  <?php
                ...
                /* if (  get_bk_option( 'booking_dif_colors_approval_pending'   ) == 'Off') { ?>
<style type="text/css">
...
//$start_script_code .= "<style type='text/css'> .tooltips{ font-family:Arial; font-size:12px; font-weight:normal;  line-height:16px; } </style>";
            ...
            $my_result =  $script_code .' ' . $this->get__client_side_booking_content($my_boook_type, $my_booking_form, $my_selected_dates_without_calendar ) . ' ' . $start_script_code ;
        ...
        function get__client_side_booking_content($my_boook_type = 1 , $my_booking_form = 'standard', $my_selected_dates_without_calendar = '') {
            ...
            if (get_bk_option( 'booking_is_show_legend' ) == 'On') { //TODO: check here according legend
                ...
                if ($this->wpdev_bk_pro !== false)
                    if ($this->wpdev_bk_pro->wpdev_bk_premium !== false)
                        $calendar .= '<div class="wpdev_hint_with_text"><div class="block_time timespartly">#</div><div class="block_text">- '.__('Partially booked','wpdev-booking') .'</div></div>';
        ...
        function wpdev_booking_activate() {
            ...
            if ( ! $this->is_table_exists('booking') ) { // Cehck if tables not exist yet
                ...
                $simple_sql = "ALTER TABLE ".$wpdb->prefix ."booking ADD form TEXT AFTER booking_id";
        ...
        function wpdev_booking_deactivate() {
            ...
            if ($is_delete_if_deactive == 'On') {
                ...
                if ( false === $wpdb->query( "DELETE FROM ". $wpdb->usermeta ." WHERE meta_key LIKE '%booking_%'") ) {    // All users data
                    echo __('Error during deleting user meta at DB', 'wpdev-booking');
                    die();
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function get_form_content ($formdata, $bktype =-1 , $booking_form_show ='' ) {
        ...
        for ( $i=0 ; $i < $formdata_array_count ; $i++) {
            ...
            if ( $type_name == 'secondname'    ) {
                $secondname_of_person = $value;
            }
            if ( $type_name == 'visitors') {
...
//adebug($dates_str);
    ...
    function get_times_from_bk_form($sdform, $my_dates){
...
//debuge(';Fin')                            ;die;
    ...
    function sendDeclineEmails($approved_id_str, $is_send_emeils) {
                global $wpdb;
...
//debuge($my_booking_hash)
            ...
            if ($my_booking_id>0) {                  // Edit exist booking
                ...
                $sql_insertion = "INSERT INTO ".$wpdb->prefix ."booking (form, booking_type) VALUES ('$formdata',  $bktype)" ;
            ...
            $start_end_time = get_times_from_bk_form($sdform, $my_dates);
    ...
    location.reload(true);
        ...
        case 'DELETE_APPROVE' :   //adebug($_POST); die();
            ...
            if ( (count($approved_id)>0) && ($approved_id !=false) && ($approved_id !='')) {
                ...
                sendDeclineEmails($approved_id_str, $is_send_emeils);
                /*
                $sql = "SELECT *    FROM ".$wpdb->prefix ."booking as bk
                                    WHERE bk.booking_id IN ($approved_id_str)";
