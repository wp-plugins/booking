...
//check compatibility xLanguage plugin
 */


function wpdev_bk_define_static() {
    if (!defined('WPDEV_BK_VERSION'))    define('WPDEV_BK_VERSION',  '1.3.3' );                                // 0.1
...
if (!class_exists('wpdev_booking')) {
	...
	function js_escape($text) {
        ...
        function print_js_at_footer(){ 
          if ( ( strpos($_SERVER['REQUEST_URI'],'booking/wpdev-booking.phpwpdev-booking')!==false) &&
               ( strpos($_SERVER['REQUEST_URI'],'booking/wpdev-booking.phpwpdev-booking-reservation')===false )
             )   {
                ...
                foreach ($dates_to_approve as $date_to_approve) {
                      $start_script_code .= "  date2approve[". $bk_type. "][ '". ($date_to_approve[1]+0). "-" . ($date_to_approve[2]+0). "-". $date_to_approve[0]. "' ] = [".
                ...
                foreach ($dates_approved as $date_to_approve) {
                    $start_script_code.=" date_approved_par['".($date_to_approve[1]+0)."-".($date_to_approve[2]+0)."-".($date_to_approve[0])."'] = [".
                                                               ($date_to_approve[1]+0).",".($date_to_approve[2]+0).",".($date_to_approve[0]+0)."];";
                }
       ...
       // Print     J a v a S cr i p t   &    C S S    scripts for admin and client side.
        function print_js_css($is_admin =1 ) { 
           ...
           if($is_admin) { ?> <link href="<?php echo WPDEV_BK_PLUGIN_URL; ?>/css/admin.css" rel="stylesheet" type="text/css" />  <?php }
               ...
               if (  get_option( 'booking_dif_colors_approval_pending'   ) == 'On') { ?>
               <style type="text/css">
                   .datepick  .date2approve  { color:#ea5; }
                   .datepick  .date_approved { color:#79c; }
               </style> <?php 
               }
           }
