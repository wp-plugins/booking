...
/*
Tested WordPress Versions: 2.8.3 - 2.9.2
-----------------------------------------------
Change Log and Features for Future Releases :
-----------------------------------------------
TODOList:
 * Showing booking from all booking types at admin panel, agregation of data into 1 calendar from severeal booking resources.
 ...
 * Ability to implement the the price in the mail
 * Mail to the customer after buying
 * More advances pricing system: 5 day discount, check box for extra fees (insurance, extra towels), - Discounts for packages of days in specific seasons. The above prices cambiarl need for different seasons. - Discounts for packages of days in specific seasons
 * Customization in form every resource (im renting out boats and rooms – they need different forms )
 * Multi language function
 * easy way of keeping track of what is agreed with the client, so either the possibility of editing the information the client has stored or to add a notes field would be superb. (in admin)
 * an easier way of managing the bookings, say to sort the bookings in order by date, or/and a hyperlink from the calendar to the booking (or a popup) (also admin)
 * a checkbox to enable sandbox testing, while you are in testing mode. This would enable the developer to try and do a complete booking with payment, without actually paying anything. Just a nice little extra touch.
 * to have one calendar for a lot of ressource with the same type ?
 ...
 *  the ability to have a  page with the booking list  for the connected user and the possibility to cancel his bookings one by one
 * automatic expiration/cancellation when bookings aren't being confirmed within a certain timeframe
 * bookings be rescheduled? If so, when a booking is rescheduled, the system send an e-mail to the customer
 ...
 * send text messages after booking appointments, send reminders via email and text messaging
 * setting management of each resource : Ressource1 , cost 20, cost period : 1 hour ;   Ressource2 , cost 40 cost period : 1 day
 * new feature to make sure the starting day of the reservation is the same (for examplemonday) in dynamic range selection and also the rent time (customers can still rent a house for 3 days for instance)..
 * fixed booking times are: 5am-5:45,  5:45-6:30,  6:30-7:15, , 7:15-8:00, ....
 * Advanced cost magement, posibility to add additional cost (for example some deposit, or cost depends from number of visitors or request of some other services). Addition  rental fee, (for exmaple for using some object).
 * More easy selection/editing  of date format - inserting different hotels in website and each one with its form:  so every form should send booking request email to a different address
 * SMS or text message option, to notify you immediately via your cell phone when you get a booking online
 ...
 * ossible to not allow the booking before the paypal payment has been made, meaning, to entry in the booking calendar, no email, etc. before the payment has been confirmed via paypal
 * Add to the popup dialog selection of shortcode for showing only calendar from pages / posts
 * administrator (or whomever receives the email) confirm the booking from the email without having to go into the backend of wordpress. i.e. The email goes to my client to confirm the booking, I don’t want them logging into wordpress to confirm etc
 * On the Hotel addition, I would suggest an option to input ACTUAL prices directly for the “Seasonal Filters” as I can’t get to an even price using a percentage. For example, base price is $135 + filter of 11.11% = $149.9985 — I need to get the price to $150 EVEN, I would rather just input the price than use a percentage.
...
<style type='text/css'>
    ...
    #wpdev_bk-dialog .dialog-wraper {float:left;width:100%;}
        ...
        function get_dates ($approved = 'all', $bk_type = 1) {
            ...
            $dates_approve = apply_bk_filter('get_bk_dates', $wpdb->get_results( $sql_req ), $approved );
            ...
            if ( count($result) > 0 ) {
                ...
                <table class='booking_table' cellspacing="0">
                    <tr>
                        ...
                        <th><?php _e('Dates', 'wpdev-booking'); ?><div style="float:right;">
                ...
                <?php
                    foreach ($result as $bk) {
                        ...
                        $date_class = ( date('m',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('d',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('Y',  mysql2date('U',$bk->booking_date)) + 0 );
                        if ($old_id !== $bk->booking_id ) { //add NEW full row

                            // Close previos roW
                            if ($old_id !=='') {

                                if ( $last_date !== $last_show_date ) {// Last day and last show day is not the same day
                                    $date_class1 = ( date('m',  mysql2date('U',$last_date)) + 0 ) . '-' . ( date('d',  mysql2date('U',$last_date)) + 0 ) . '-' . ( date('Y',  mysql2date('U',$last_date)) + 0 );
                                    $short_dates_view .= '<span class="date_tire"> - </span>' . '<span class="adate-'.$date_class1.'"><a href="#" class="booking_overmause'. $outColorClass.'"' .
                                                                                       ' onmouseover="javascript:highlightDay(\'cal4date-' . $date_class1. '\',\'#ff0000\');" ' .
                                                                                       ' onmouseout="javascript:highlightDay(\'cal4date-' . $date_class1. '\',\''.$outColor .'\');" ' . ' >' . $this->get_showing_date_format( mysql2date('U', $last_date ) ) .
                                            '</a></span>';
                                    $short_dates_view .= apply_bk_filter('filter_subtype_num','',$bk_type, $old_id, $last_date);
                                    $short_dates_view .= ' ';
                                }
                                $short_dates_view .= '</div>';

                                echo "</div>"; // close   W I D E   DIV view

                                echo $short_dates_view;
                                echo "</td><td ".$alternative_color." >";
                                if( $this->wpdev_bk_pro !== false ) echo "<input type='button' class='button' value='&nbsp;&nbsp;&nbsp;".__('Edit','wpdev-booking')."&nbsp;&nbsp;&nbsp;' onclick='javascript:location.href=&quot;admin.php?page=".WPDEV_BK_PLUGIN_DIRNAME . '/'. WPDEV_BK_PLUGIN_FILENAME ."wpdev-booking-reservation&booking_type=".$bk_type."&booking_id=".$old_id."&quot;;'>";
                                else                                echo "<input type='button' class='button' value='&nbsp;&nbsp;&nbsp;".__('Edit','wpdev-booking')."&nbsp;&nbsp;&nbsp;' onclick='javascript:openModalWindow(&quot;modal_content1&quot;);'>";
                                if( $this->wpdev_bk_pro !== false ) echo "<input type='button' class='button' value='".__('Remark','wpdev-booking')."' onclick='javascript:if (document.getElementById(\"remark_row".$old_id."\").style.display==\"block\") document.getElementById(\"remark_row".$old_id."\").style.display=\"none\"; else document.getElementById(\"remark_row".$old_id."\").style.display=\"block\"; '>";
                                else                                echo "<input type='button' class='button' value='".__('Remark','wpdev-booking')."' onclick='javascript:openModalWindow(&quot;modal_content1&quot;);'>";

                                make_bk_action('show_remark_editing_field',$old_id, $previos_object_row, "height:1px;padding:0px;margin:0px;border-bottom:1px solid #ccc;" );

                                echo "</td></tr>";

                                // Start new Date
                                $short_dates_view = '<div id="short_dates_view'.$bk->booking_id.'"  class="short_dates_view'. $short_days_class. '"  >';
                                $short_dates_view .= '<span class="adate-'.$date_class.'"><a href="#" class="booking_overmause'. $outColorClass.'"'.
                                                                                       ' onmouseover="javascript:highlightDay(\'cal4date-' . $date_class. '\',\'#ff0000\');" ' .
                                                                                       ' onmouseout="javascript:highlightDay(\'cal4date-' . $date_class. '\',\''.$outColor .'\');" ' . ' >' . $this->get_showing_date_format( mysql2date('U',$bk->booking_date) ) .
                                        '</a></span>';
                                $short_dates_view .= apply_bk_filter('filter_subtype_num','',$bk_type, $bk->booking_id, $bk->booking_date);
                                $short_dates_view .= ' ';
                                $last_show_date = $bk->booking_date;
                                $last_date = $bk->booking_date;

                            } else { // Start - first call of dates
                                $short_dates_view = '<div id="short_dates_view'.$bk->booking_id.'" class="short_dates_view'. $short_days_class. '"  >';
                                $short_dates_view .= '<span class="adate-'.$date_class.'"><a href="#" class="booking_overmause'. $outColorClass.'"'.
                                                                                       ' onmouseover="javascript:highlightDay(\'cal4date-' . $date_class. '\',\'#ff0000\');" ' .
                                                                                       ' onmouseout="javascript:highlightDay(\'cal4date-' . $date_class. '\',\''.$outColor .'\');" ' . ' >' . $this->get_showing_date_format( mysql2date('U',$bk->booking_date) ) .
                        ...
                        ?>
                            <tr id="booking_row<?php  echo $bk->booking_id  ?>"  <?php if ($_GET['booking_id_selection'] ==$bk->booking_id) echo ' class="raw_after_editing" '; ?>  >
                                ...
                                <td <?php echo $alternative_color; ?> style="text-align:center;" > <?php make_bk_action('show_remark_hint',$bk->booking_id, $bk);  echo $bk->booking_id;  ?> </td>
                                <td <?php echo $alternative_color; ?> > <?php $cont = get_form_content($bk->form) ; ?>
                                ...
                                <td <?php echo $alternative_color; ?> >
                                    <?php // Add start date ?>
                                    <?php  $date_class = ( date('m',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('d',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('Y',  mysql2date('U',$bk->booking_date)) + 0 );  ?>
                                    <div id="wide_dates_view<?php echo $bk->booking_id;?>" class="wide_dates_view<?php echo $wide_days_class; ?>"  >
                                    ...
                                    make_bk_action('show_subtype_num',$bk_type, $bk->booking_id, $bk->booking_date);
                        } else {   // add only dates
                            echo ', '; ?>
                                    <?php  $date_class = ( date('m',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('d',  mysql2date('U',$bk->booking_date)) + 0 ) . '-' . ( date('Y',  mysql2date('U',$bk->booking_date)) + 0 );  ?>
                                    <span class="<?php echo "adate-" . $date_class; ?>"><a href="#" class="booking_overmause<?php echo $outColorClass; ?>"
                                    ...
                                    if ( $this->is_next_day( $bk->booking_date ,$last_date) ) { // Found next day do not make anything
                                        $last_date = $bk->booking_date;
                                    } else {                                                    // Found NOT next day, need to show
                                        if ( $last_date !== $last_show_date ) { // Last day and last show day is not the same day
                                            $date_class1 = ( date('m',  mysql2date('U',$last_date)) + 0 ) . '-' . ( date('d',  mysql2date('U',$last_date)) + 0 ) . '-' . ( date('Y',  mysql2date('U',$last_date)) + 0 );
                                            $short_dates_view .= '<span class="date_tire"> - </span>' . '<span class="adate-'.$date_class1.'"><a href="#" class="booking_overmause'. $outColorClass.'"'.
                                                                                       ' onmouseover="javascript:highlightDay(\'cal4date-' . $date_class1. '\',\'#ff0000\');" ' .
                        ...
                        $previos_object_row = $bk;
                    }
                    echo "</div>";  // close   W I D E   DIV view

                    if ( $last_date !== $last_show_date ) {// Last day and last show day is not the same day
                        $short_dates_view .= '<span class="date_tire"> - </span>' . '<span class="adate-'.$date_class.'"><a href="#" class="booking_overmause'. $outColorClass.'"'.
                                                                                       ' onmouseover="javascript:highlightDay(\'cal4date-' . $date_class. '\',\'#ff0000\');" ' .
                                                                                       ' onmouseout="javascript:highlightDay(\'cal4date-' . $date_class. '\',\''.$outColor .'\');" ' . ' >' . $this->get_showing_date_format( mysql2date('U',$bk->booking_date) ) .
                                '</a></span>';
                        $short_dates_view .= apply_bk_filter('filter_subtype_num','',$bk_type, $bk->booking_id, $bk->booking_date);
                   ...
                   make_bk_action('show_remark_editing_field',$old_id,$previos_object_row,"height:1px;padding:0px;margin:0px;border:none;");
                  ?>  </td></tr></table> <?php
                    return true;
                } else {
                    return false;
...
<?php if ( ($version == 'free') || ($version == 'premium') || ($version == 'hotel') ) { ?>
 ...
 <a href="admin.php?page=<?php echo WPDEV_BK_PLUGIN_DIRNAME . '/'. WPDEV_BK_PLUGIN_FILENAME ; ?>wpdev-booking-option&tab=paypal" class="bk_top_menu <?php echo $slct_a; ?>"><?php $title = __('Paypal and cost', 'wpdev-booking'); echo $title; if ($slct_a == 'selected') { $selected_title = $title; $selected_icon = 'Paypal-cost-64x64.png'; } ?></a>
            <?php
 }
                ...
                <div class="inside">
                    ...
                    <div class="table">
                        <table><tbody>
                                ...
                                <tr class="last">
                                    <td class="first b" ><?php _e("Online demo", 'wpdev-booking'); ?></td>
...
<div  style="width:99%; ">
   ...
   <h2><?php printf(__('This functionality exist at the %sBooking Calendar%s %sHotel Edition%s', 'wpdev-booking'),'<span style="color:#f50;">','</span>', '<a href="http://hotel.onlinebookingcalendar.com/wp-admin/" style="color:#FF5500;">','</a>'); ?></h2>
    ...
    <h2><?php printf(__('Check %sfeatures%s, test %sonline demo%s and %sbuy%s.', 'wpdev-booking'),
        ...
        function get_booking_form_action($my_boook_type=1,$my_boook_count=1){

            $res = $this->add_booking_form_action($my_boook_type,$my_boook_count, 0 );
            return $res;
...
//echo $start_script_code; die;
        ...
        function wpdev_booking_activate() {
            ...
            add_option( 'booking_email_reservation_content',htmlspecialchars(sprintf(__('You need to approve new reservation %s at dates: %s Person detail information:%s Thank you, Booking service.%s', 'wpdev-booking'),'[bookingtype]','[dates]<br/><br/>','<br/> [content]<br/><br/>','<br/>[siteurl]')));
            ...
            add_option( 'booking_email_approval_content',htmlspecialchars(sprintf(__('Your reservation %s at dates: %s has been approved.%sThank you, Booking service.%s', 'wpdev-booking'),'[bookingtype]','[dates]','<br/><br/>[content]<br/><br/>','<br/>[siteurl]')));
            ...
            add_option( 'booking_email_deny_content',htmlspecialchars(sprintf(__('Your reservation %s at dates: %s has been  canceled. %sThank you, Booking service.%s', 'wpdev-booking'),'[bookingtype]','[dates]','<br/><br/>[denyreason]<br/><br/>[content]<br/><br/>','<br/>[siteurl]')));
...
function get_form_content ($formdata, $bktype =-1 ) {
    ...
    if ($bktype == -1) {
        if ( isset($_GET['booking_type']) ) { $bktype = $_GET['booking_type']; }
        else                                { $bktype = '1'; }
    }
    ...
    for ( $i=0 ; $i < $formdata_array_count ; $i++) {
        ...
        //debuge( '['. $type_name .']', $value ,$booking_form_show);
    }
