...
/*
Tested WordPress Versions: 2.8.3 - 2.8.5
-----------------------------------------------
 ...
 * Captcha to the form
 * Dublicated emeils sending to cpecific emeil for audit of each reservation or approval or cancellation (pro version).

= 1.4 =
 * Professional and Delux version features:
  * Divide day to the parts according to the booking time (delux version)
...
function wpdev_bk_define_static() {
    if (!defined('WPDEV_BK_VERSION'))    define('WPDEV_BK_VERSION',  '1.2' );                                // 0.1
    ...
    // Localization
    if ( ! loadLocale() )  loadLocale('en_EN');
}
...
if (!class_exists('wpdev_booking')) {
    class wpdev_booking   {
        ...
        function wpdev_booking() {
            ...
            $this->gui_booking = &new wpdev_gui_bk('wpdev-booking',__FILE__,__('Bookings service'), __('Bookings'),'');
            ...
            $this->gui_reservation = &new wpdev_gui_bk('wpdev-booking-reservation',__FILE__  ,__('Add booking'), __('Add booking'),'submenu','', __FILE__ . $this->gui_booking->gui_html_id_prefix,0);
            ...
            $this->gui_settings = &new wpdev_gui_bk('wpdev-booking-option',__FILE__  ,__('Booking settings'), __('Settings'),'submenu','', __FILE__ . $this->gui_booking->gui_html_id_prefix,1);
                            ...
                            // <![CDATA[
                                ...
                                function booking_click( tag ) {
                                    ...
                                    jWPDev(".ui-dialog button").addClass("button").each(function(){
                                        if ( "<?php echo js_escape(__('Ok')); ?>" == jWPDev(this).html() ) jWPDev(this).addClass("button-highlighted");
                                    });
                                                ...
                                                <div class="wpdev_bk-dialog-inputs">
                                                    ...
                                                    ?>
                                                        <div class="field">
                                                            ...
                                                            <span class="description"><?php _e('Enter type of property for booking.'); ?></span>
                                                        </div>
                                                    <?php }/**/ ?>
                                                    <div class="field">
	...
	function js_escape($text) {
        ...
        function get_showing_date_format($mydate ){
           ...
           return date($date_format, $mydate);
        ...
        function booking_table($approved = 0) {
            ...
            <table class='booking_table' cellspacing="0">
                <tr>
                    <th style="width:70px;"><?php _e('Approved'); ?></th>
                    <th style="width:15px;"><?php _e('ID'); ?></th>
                    <th style="width:49%;"><?php _e('Info'); ?></th>
                    <th><?php _e('Dates'); ?></th>
                </tr>
                                <?php
        ...
        //   A D M I N     S I D E   /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        function show_booking_widget($args){
            extract($args);
            ...
            echo $before_widget;
            echo $before_title . $booking_widget_title . $after_title;
        ...
        function booking_widget_control(){
            ...
            <p>
            <label><?php _e('Show'); ?>:</label><br/>
             <select id="booking_widget_show" name="booking_widget_show"  style="width:100%;">
                <option <?php if($booking_widget_show == 'booking_calendar') echo "selected"; ?> value="booking_calendar"><?php _e('Booking calendar'); ?></option>
                <option <?php if($booking_widget_show == 'booking_form') echo "selected"; ?> value="booking_form"><?php _e('Booking form'); ?></option>
             </select>
            </p>
            ...
            ?>
            <p>
                <label><?php _e('Booking property'); ?>:</label><br/>
                    <!--input id="calendar_type"  name="calendar_type" class="input" type="text" -->
            ...
            <p>
            <label><?php _e('Count of calendars'); ?>:</label><br/>
            ...
            <label><?php printf(__("Example: %sMake booking here%s"),"<code>&lt;a href='LINK'&gt;",'&lt;/a&gt;</code>'); ?></label>
            </p>

            <p>
            <?php printf(__("%sImportant!!!%s Please note, if you show booking calendar (inserted into post/page) with booking calendar widget at the same page, then widget booking calendar will not visible."),'<strong>','</strong><br/>'); ?>
            </p>
            <?php  
        }
        ...
        function add_booking_widjet(){
            ...
            $widget_options = array('classname' => 'widget_wpdev_booking', 'description' => __( "Display Booking.") );
            wp_register_sidebar_widget('wpdev_booking_widget', __('Booking'), array(&$this,'show_booking_widget'), $widget_options);
            wp_register_widget_control('wpdev_booking_widget', __('Booking settings'), array(&$this, 'booking_widget_control') );
        }
        ...
        function print_js_at_footer(){ 
          if ( (strpos($_SERVER['REQUEST_URI'],'booking.phpwpdev-booking')!==false) && ( strpos($_SERVER['REQUEST_URI'],'booking/wpdev-booking.phpwpdev-booking-reservation')===false ) )   {
                ...
                $start_script_code .= " jWPDev('.wpdev-validates-as-time').attr('alt','time');";  //add mask to the time field

                $start_script_code .= "  date2approve[". $bk_type. "] = [];";
                $start_script_code .= "  date_approved[". $bk_type. "] = [];";
                $dates_to_approve = $this->get_dates('0', $bk_type);
                foreach ($dates_to_approve as $date_to_approve) {
                      $start_script_code .= "  var d_to_appr = '". ($date_to_approve[1]+0). "-" . ($date_to_approve[2]+0). "-". $date_to_approve[0]. "';";
                      $start_script_code .= "  date2approve[". $bk_type. "][d_to_appr] = [". ($date_to_approve[1]+0).", ". ($date_to_approve[2]+0).", ". ($date_to_approve[0]+0). "];";
                }

                $dates_approved = $this->get_dates('1', $bk_type);
                foreach ($dates_approved as $date_to_approve) {
                        $start_script_code .= "  var d_to_appr = '". ($date_to_approve[1]+0). "-" . ($date_to_approve[2]+0). "-". $date_to_approve[0]. "';";
                        $start_script_code .= "  date_approved[". $bk_type. "][d_to_appr] = [". ($date_to_approve[1]+0).", ". ($date_to_approve[2]+0).", ". ($date_to_approve[0]+0). "];";
                }

                $start_script_code .= "     init_datepick_cal('". $bk_type ."',   date_approved_par, ".get_option( 'booking_admin_cal_count' ).", ". get_option( 'booking_start_day_weeek' ) . " ); });        // Connect init function";
       ...
       // Print     J a v a S cr i p t   &    C S S    scripts for admin and client side.
        function print_js_css($is_admin =1 ) { 
           ...
           ?> <!-- Booking Calendar Scripts -->
                    <script  type="text/javascript">
                        ...
                        var message_verif_requred = '<?php _e('This field is required'); ?>';
                        var message_verif_emeil = '<?php _e('Incorrect email field'); ?>';
                        var message_verif_selectdts = '<?php _e('Please, select reservation date(s) at Calendar.'); ?>';
           ...
           <div id="ajax_working"></div>
                ...
                <div id="table_for_approve">
                    ...
                    <div style="float:left;">
                        ...
                        <input class="button-primary" type="button" style="margin:20px 10px;" value="<?php _e('Cancel'); ?>"  onclick="javascript:bookingApprove(1,0);"/>
                    </div>
                    <div style="float:left;border:none;margin-top:18px;"> <input type="text" style="width:350px;float:left;" name="denyreason" id="denyreason" value="<?php _e('Reason of cancellation here'); ?>"> </div>
            ...
            <div id="table_approved">
                <div class="reserved_mark"></div><h2><?php _e('Reserved'); ?></h2>
                ...
                <div style="float:left;border:none;margin-top:18px;"> <input type="text" style="width:350px;float:left;" name="cancelreason" id="cancelreason" value="<?php _e('Reason of cancellation here'); ?>"> </div>
            ...
            <div  class="copyright_info" style="">
                <div style="width:580px;height:10px;margin:auto;">
                    ...
                    <div style="margin: 2px 0px 0px 20px; float: left; text-align: center;">
                        ...
                        <a href="http://www.wpdevelop.com" target="_blank" style="text-decoration:underline;"  valign="middle">www.wpdevelop.com</a> <?php _e(' - custom wp-plugins and wp-themes development, WordPress solutions');?><br>
                    </div>
                </div>
            </div>
                                                ...
                                                <table class="form-table"><tbody>
                                                        ...
                                                        <th scope="row"><label for="client_cal_count" ><?php _e('Count of calendars'); ?>:</label><br><?php printf(__('at %sclient%s side view'),'<span style="color:#888;font-weight:bold;">','</span>'); ?></th>
                                                            <td><input id="client_cal_count" class="regular-text code" type="text" size="45" value="<?php echo $client_cal_count; ?>" name="client_cal_count"/>
                                                                <span class="description"><?php printf(__('Type your %sdefault count of calendars%s for inserting into post/page'),'<b>','</b>');?></span>
                                                            </td>
                                                        </tr>
                                                        ...
                                                        <tr valign="top">
                                                        <th scope="row"><label for="booking_date_format" ><?php _e('Date Format'); ?>:</label><br><?php printf(__(''),'<span style="color:#888;font-weight:bold;">','</span>'); ?></th>
                                                            <td><input id="booking_date_format" class="regular-text code" type="text" size="45" value="<?php echo $booking_date_format; ?>" name="booking_date_format"/>
                                                                <span class="description"><?php printf(__('Type your date format for showing in emeils and booking table. %sDocumentation on date formatting.%s'),'<br/><a href="http://codex.wordpress.org/Formatting_Date_and_Time" target="_blank">','</a>');?></span>
                                                            </td>
                                                        </tr>

                                                        <tr valign="top">
                                                        <th scope="row"><label for="new_booking_title" ><?php _e('New reservation title'); ?>:</label><br><?php printf(__('%sshowing after%s booking'),'<span style="color:#888;font-weight:bold;">','</span>'); ?></th>
                                                        <td><input id="new_booking_title" class="regular-text code" type="text" size="45" value="<?php echo $new_booking_title; ?>" name="new_booking_title" style="width:99%;"/>
                                                                <br/><span class="description"><?php printf(__('Type title of new reservation %safter booking is done by user%s'),'<b>','</b>');?></span>
                                                            </td>
                                                        </tr>
                                                        ...
                                                        <tr valign="top">
                                                            ...
                                                            <td><input id="wpdev_copyright" type="checkbox" <?php if ($wpdev_copyright == 'On') echo "checked"; ?>  value="<?php echo $wpdev_copyright; ?>" name="wpdev_copyright"/>
                                                                <span class="description"><?php printf(__(' Turn On/Off copyright %s notice at footer of site view.'),'wpdevelop.com');?></span>
                                                            </td>
                                                        </tr>
                                ...
                                <div  class="postbox gdrgrid" > <h3 class='hndle'><span><?php _e('Professional version'); ?></span></h3>
                                    <div class="inside">
                                        ...
                                        <div class="table">
                                            <table><tbody>
                                                <tr class="first">
                                                    <td class="first b" style="width: 53px;"><b><?php _e("Features"); ?></b></td>
                                                    <td class="t"></td>
                                                    <td class="b options comparision" style="color:#11cc11;"><b><?php _e("Free"); ?></b></td>
                                                    <td class="b options comparision" style="color:#f71;"><b><?php _e("Pro"); ?></b></td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b" ><?php _e("Functions"); ?></td>
                                                    <td class="t i">(<?php _e("all existing functions"); ?>)</td>
                                                ...
                                                <tr class="">
                                                    ...
                                                    <td class="t i">(<?php _e("several bookings on the same date"); ?>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b"><?php _e("Time"); ?></td>
                                                    <td class="t i">(<?php _e("enter the booking time"); ?>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b"><?php _e("Form"); ?></td>
                                                    <td class="t i">(<?php _e("field customization"); ?>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b"><?php _e("Email"); ?></td>
                                                    <td class="t i">(<?php _e("email customization"); ?>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b"><?php _e("Range selection"); ?></td>
                                                    <td class="t i">(<?php _e("selection fixed several days"); ?>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr class="">
                                                    <td class="first b"><?php _e("Payment"); ?></td>
                                                    <td class="t i">(<strong><?php _e("PayPal support"); ?></strong>)</td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;"></td>
                                                    <td class="b options comparision" style="text-align:center;color: green; font-weight: bold;">&bull;</td>
                                                </tr>
                                                <tr>
                                                    ...
                                                    <td class="t options comparision"><?php _e("start"); ?> $99</td>
                                                </tr>
                                                <tr class="last">
                                                ...
                                                <tr class="last">
                                                    <td class="first b"><span><?php _e("Purchase"); ?></span></td>
                                                    <td class="t i">(<?php _e("buy Professional verion"); ?>)</td>
                                                    <td colspan="2" style="text-align:right;" class="t comparision"><strong><a  href="http://wpdevelop.com/booking-calendar-professional/" target="_blank"><?php _e("Buy now"); ?></a></strong></td>
                                                </tr>
                                    ...
                                    <div class="inside">
                                        ...
                                        <div class="table">
                                            <table><tbody>
                                                ...
                                                <tr class="first">
                                                    <td class="first b"><?php _e('Demo page'); ?></td>
                                                ...
                                                <tr>
                                                    <td class="first b"><?php _e('WordPress Extend'); ?></td>
        ...
        function add_calendar_action($bk_type =1, $cal_count =1){
            ...
            $start_script_code .= "     init_datepick_cal('". $my_boook_type ."', date_approved_par, ". $my_boook_count ." , ". get_option( 'booking_start_day_weeek' ) . "); });        // Connect init function";
            $start_script_code .= "</script>";
            echo $script_code .' '
        ...
        function add_booking_form_action($bk_type =1, $cal_count =1){
            ...
            $start_script_code .= "     init_datepick_cal('". $my_boook_type ."', date_approved_par, ". $my_boook_count ." , ". get_option( 'booking_start_day_weeek' ) . "); });        // Connect init function";
            $start_script_code .= "</script>";
            echo $script_code .' ' . $this->get__client_side_booking_content($my_boook_type ) . ' ' . $start_script_code ;
        ...
        function get_booking_form($my_boook_type){
            return  '<div style="text-align:left;">
                    <p>'.__('Name (required)').':<br />  <span class="wpdev-form-control-wrap name'.$my_boook_type.'"><input type="text" name="name'.$my_boook_type.'" value="" class="wpdev-validates-as-required" size="40" /></span> </p>
                    <p>'.__('Second Name (required)').':<br />  <span class="wpdev-form-control-wrap secondname'.$my_boook_type.'"><input type="text" name="secondname'.$my_boook_type.'" value="" class="wpdev-validates-as-required" size="40" /></span> </p>
                    <p>'.__('Email (required)').':<br /> <span class="wpdev-form-control-wrap email'.$my_boook_type.'"><input type="text" name="email'.$my_boook_type.'" value="" class="wpdev-validates-as-email wpdev-validates-as-required" size="40" /></span> </p>
                    <p>'.__('Phone').':<br />            <span class="wpdev-form-control-wrap phone'.$my_boook_type.'"><input type="text" name="phone'.$my_boook_type.'" value="" size="40" /></span> </p>
                    <p>'.__('Details').':<br />          <span class="wpdev-form-control-wrap details'.$my_boook_type.'"><textarea name="details'.$my_boook_type.'" cols="40" rows="10"></textarea></span> </p>
                    <p><input type="button" value="'.__('Send').'" onclick="mybooking_submit(this.form,'.$my_boook_type.');" /></p>
                    </div>';
        }
        ...
        function get__client_side_booking_content($my_boook_type = 1 ) {
            ...
            return apply_filters('wpdev_booking_form','<form name="booking_form'.$my_boook_type.'" class="booking_form" id="booking_form'.$my_boook_type.'"  method="post" action=""><div id="ajax_respond_insert'.$my_boook_type.'"></div>' .
                    $res . '</form>');
        }
        ...
        function wp_footer(){
            if ( ( get_option( 'booking_wpdev_copyright' )  == 'On' ) && (! defined('WPDEV_COPYRIGHT')) ) {
                        printf(__('%sPowered by wordpress plugins developed by %'),'<span style="font-size:9px;text-align:center;margin:0 auto;">','<a href="http://www.wpdevelop.com" target="_blank">swww.wpdevelop.com</a></span>','&amp;');
        ...
        function wpdev_booking_activate() {
           ...
           add_option( 'booking_title_after_reservation' , sprintf(__('Thank you for your online reservation request. %s We will make contact to confirm your booking as soon as possible.'), '<br/>') );
           ...
           add_option( 'booking_email_reservation_subject',__('New reservation'));
           add_option( 'booking_email_reservation_content',htmlspecialchars(sprintf(__('You need to approve new reservation %s at dates: %s Person detail information:%s Thank you, Booking service.'),'[bookingtype]','[dates]<br/><br/>','<br/> [content]<br/><br/>')));

           add_option( 'booking_email_approval_adress',htmlspecialchars('"Booking system" <' .get_option('admin_email').'>'));
           add_option( 'booking_email_approval_subject',__('Your reservation has been approved'));
           add_option( 'booking_email_approval_content',htmlspecialchars(sprintf(__('Your reservation %s at dates: %s has been approved.%sThank you, Booking service.'),'[bookingtype]','[dates]','<br/><br/>[content]<br/><br/>')));

           add_option( 'booking_email_deny_adress',htmlspecialchars('"Booking system" <' .get_option('admin_email').'>'));
           add_option( 'booking_email_deny_subject',__('Your reservation has been declined'));
           add_option( 'booking_email_deny_content',htmlspecialchars(sprintf(__('Your reservation %s at dates: %s has been  canceled. %sThank you, Booking service.'),'[bookingtype]','[dates]','<br/><br/>[denyreason]<br/><br/>[content]<br/><br/>')));


           add_option('booking_widget_title', __('Booking form') );
...
function get_booking_form_show(){
    return '<div style="text-align:left">
            ...
            <strong>'.__('Second Name').'</strong>:<span class="fieldvalue">[secondname]</span><br/>
...
function wpdev_bk_ajax_responder() {
   ...
   // Change date format
   function change_date_format($mydates, $is_day_before_month = false){
   ...
   function get_dates_str ($approved_id_str) {
    ...
    switch ( $action ) :
        ...
        case  'INSERT_INTO_TABLE':
            ...
            $is_day_before_month = true;
            $mail_body = str_replace('[dates]', change_date_format(str_replace('|',',',$dates), $is_day_before_month), $mail_body);
            ...
            <script type="text/javascript">
               ...
               jWPDev('.submiting_content').fadeOut(7000);
               setReservedSelectedDates('<?php echo $bktype; ?>');
            </script>
            <?php
            do_action('wpdev_new_booking',$booking_id, $bktype, str_replace('|',',',$dates));
            die();
        ...
        case 'DELETE_APPROVE' :   // adebug($_POST); die();
            ...
            if ( ( $denyreason == __('Reason of cancellation here')) || ( $denyreason == 'Reason of cancel here') ) {
                $denyreason = '';
            }
