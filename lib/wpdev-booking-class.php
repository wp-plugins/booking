...
class wpdev_booking {
    ...
    function wpdev_booking() {
        ...
        if(defined('WP_ADMIN')){
            ...
            if ( version_compare(WP_BK_VERSION_NUM, $booking_version_num) > 0 ){
                ...
                if ( ( class_exists('wpdev_bk_personal') ) && ( ! $this->is_table_exists('bookingtypes') )  ) 
                    $is_make_activation = true;
                
                if ( (! $is_make_activation) && ( class_exists('wpdev_bk_biz_s') ) && ($this->is_field_in_table_exists('booking','pay_request') == 0) )     
                    $is_make_activation = true;
                                
                if ( (! $is_make_activation) && ( class_exists('wpdev_bk_biz_m') ) && ( ! $this->is_table_exists('booking_types_meta') )  ) 
                    $is_make_activation = true;
                
                if ( (! $is_make_activation) && ( class_exists('wpdev_bk_biz_l') ) && ( ! $this->is_table_exists('booking_coupons') )  )                 
                    $is_make_activation = true;
                
                if ( (! $is_make_activation) && ( class_exists('wpdev_bk_multiuser') ) && ($this->is_field_in_table_exists('booking_coupons','users') == 0) ) 
                    $is_make_activation = true;
                ...
                <div class="bk_dashboard_section border_orrange" id="bk_upgrade_section"> <!-- Section 2 -->
                    <div style="padding:0px 10px;width:96%;">
                        ...
                        <p>
                            ...
                            <?php } if ($version == 'free') { ?>
                                <a href="http://wpbookingcalendar.com/buy/" target="_blank" class="button-primary" style="float:left;"><?php _e('Buy now', 'wpdev-booking'); ?></a> &nbsp; <?php _e('or', 'wpdev-booking'); ?> &nbsp;
                            <?php } ?>
                ...
                <table class="bk_table">
                    ...
                    <tr>
                        <td style="text-align:center;" class="bk_spec_font wpdevbk"><?php 
                            if ($version == 'free') { 
                                ?><a class="button-primary button" target="_blank" href="http://wpbookingcalendar.com/features/"><?php _e('Explore Premium Features','wpdev-booking');?></a><?php                             
                            } elseif  ($upgrade_lnk != '')  { 
    ...
    function add_new_admin_menu() {
        ...
        add_action("admin_print_styles-".   $pagehook1 , array( &$this, 'load_admin_styles_on_page') );     // This hook for loading the styles at  the header of page   
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // S E T T I N G S
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $pagehook3 = add_submenu_page(WPDEV_BK_FILE . 'wpdev-booking',__('General Settings', 'wpdev-booking'), __('Settings', 'wpdev-booking'), $users_roles[2],
    ...
    function createCapthaContent($bk_tp) {
        ...
        if ( (  get_bk_option( 'booking_is_use_captcha' ) !== 'On' ) 
                || ( strpos($_SERVER['REQUEST_URI'],'wp-admin/admin.php?') !== false ) 
           ) return '';
        else {
...
//debuge($attr);
    ...
    function wpdev_booking_activate() {
        ...
        if  ($this->is_field_in_table_exists('booking','sync_gid') == 0) {
