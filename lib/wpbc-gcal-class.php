                    ...
                    if ( ! empty( $raw_data ) ) {
                        ...
                        if ( isset( $raw_data['feed']['entry'] ) ) {
                            ...
                            foreach ( $raw_data['feed']['entry'] as $event ) {
                                ...
                                if ( (isset($event['author'])) && (isset($event['author'][0])) && (isset($event['author'][0]['email'])) && (isset($event['author'][0]['email']['$t'] )) )
                                     $event_author_email    = esc_html( $event['author'][0]['email']['$t'] );
                                else $event_author_email = '';
                                

                                if ( isset( $event['gd$when'] ) ) 
                                    list($range_dates, $range_time) = $this->getCommaSeparatedDates( $event['gd$when'] );
                                else
...
//                                                    ."text^".trim($booking_gcal_events_form_fields['where'])."{$bktype}^{$location}"
    ...
    private function getCommaSeparatedDates( $event_dates ) {
...
//debuge(  date_i18n('d.m.Y H:i', $this->iso_to_ts( $e_date['startTime'] ) )  );            
//debuge(  date_i18n('d.m.Y H:i', $this->iso_to_ts( $e_date['endTime'] ) )  );            
            
            $start_time  = $this->iso_to_ts( $e_date['startTime'] );
            
            /*
             * Google Calendar for 1 day  event return  2 days like this 04.09.2014 00:00 - 05.09.2014 00:00
             * So  we need to extract 1 date from end time to  have 04.09.2014 00:00 - 04.09.2014 23:59
             */
            if ( date_i18n('H:i', $this->iso_to_ts( $e_date['endTime'] ) ) == '00:00' ) 
                $end_time = $this->iso_to_ts( $e_date['endTime'] ) - 86400; // 24 hours - 60*60*24 = 86400
            else 
                $end_time = $this->iso_to_ts( $e_date['endTime'] );

//debuge(  date_i18n('d.m.Y H:i', $end_time )  );     

            // Loop to  add days between $start_time & $end_time
            if ( $end_time >= $start_time ) {
                
                array_push($dates, date( 'd.m.Y', $start_time ) );              // Check In day

                $start_time += 86400;                                           // add 24 hours - 60*60*24 = 86400
                while ( $start_time <= $end_time ) {
                    
                    array_push($dates, date('d.m.Y',$start_time) );
                    $start_time += 86400;                                       // add 24 hours - 60*60*24 = 86400
                }
            }            
        ...
        <table style="width:100%;margin-top:45px;" id="gcal_imported_events<?php echo $this->getResource(); ?>" class="resource_table booking_table" cellpadding="0" cellspacing="0">
            ...
            <tr class="wpbc_table_footer">
                <td colspan="6" style="text-align: center;"> 
                    <a href="javascript:void(0)" class="btn btn-primary" style="float:none;margin:10px;" 
                       onclick="javascript:location.reload();" ><?php _e('Reload page','wpdev-booking'); ?></a>                    
                    <a href="javascript:void(0)" class="btn" style="float:none;margin:10px;" 
