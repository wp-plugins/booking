                          ...
                          if ($booked_dates_array !== false) {
                               ...
                               if ( isset($booked_dates_array[ $day_filter_id ]) ) {    // This date is    B O O K E D
                                ...
                                echo '<a  href="'.$bk_url_listing.'&wh_booking_id='.$link_id_parameter.'&view_mode=vm_listing&tab=actions"
                                      ...
                                      rel="popover" class="'.write_bk_id_css_classes('a_bk_id_',$previous_booking_id).' popover_left  ' . ( ($title!='')?'first_day_in_bookin':'' ).' ">'.$title_in_day.'</a>';
                            ...
                            for ($tt = 0; $tt < $time_selles_num; $tt++) {
                                ...
                                if (empty($my_bk_id_array)) {   // Time cell  is    E m p t y
                                    ...
                                    $my_bkid_title = '<a  href="'.$bk_url_listing.'&wh_booking_id='.$link_id_parameter.'&view_mode=vm_listing&tab=actions"
                                     data-content="<div class=\'\'>'.$title_hint.'</div>"
                                     data-original-title="'.'ID: '.$title.'"
                                     rel="popover" class="'.write_bk_id_css_classes('cell_bk_id_',$previous_booking_id).' popover_left  ' . ( ($title!='')?'first_day_in_bookin':'' ).' ">'.$title_in_day.'</a>';
                                }
          ...
          <tr>
                ...
                {  // Matrix - resource titles
                    ...
                    switch ($view_days_num) {                                
                        case '90':
                            ...
                            echo __(date("M", $real_date)) . date( $date_format , $real_date) . ' - ' . __(date("M", $end_real_date)) .  date( $date_format , $end_real_date);
                            break;
            ...
            <td  style="padding:0px;">
                ...
                if ( $is_show_resources_matrix )    $resource_id = $bk_resources_id[$d_inc] ; 
                    ...
                    else $resource_id = $_GET['booking_type'];
                }
...
////   $title_hint .= ' <a href="javascript:;"  class="tooltip_bottom approve_bk_link  '. ( ($is_approved)?' hidden_items ':'') .' "
...
////   $title_hint .= '<a href="javascript:;"  class="tooltip_bottom pending_bk_link  '. ( (! $is_approved)?  ' hidden_items ':'' ) .' "
...
////    //if ( bk_are_you_sure(\''. esc_js(__('Are you really want to set booking as pending ?', 'wpdev-booking')) .'\') ) 
//   
...
//   //if ( bk_are_you_sure(\''. esc_js(__('Are you really want to delete this booking ?', 'wpdev-booking')) .'\') ) 
//   $title_hint .= '</div>';
